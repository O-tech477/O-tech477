# Hi there, I'm Ananth Ram Omkar (O-tech477) 👋

Welcome to my GitHub! I'm a passionate student and developer with a deep love for coding, web development, and machine learning. Python is my go-to language, but I'm constantly exploring new technologies to build cool things and expand my skillset.

---

## 💻 About Me

* 🎓 **Student & Lifelong Learner:** Always curious, always coding.
* 🐍 **Python Enthusiast:** My primary weapon of choice for scripting, data, and problem-solving.
* 🌐 **Web Development Fanatic:** Building responsive and dynamic web applications.
* 🤖 **Machine Learning Lover:** Diving into algorithms and data to build intelligent systems.
* 💡 **Philosophy:** Coding isn't just a skill—it's a hobby I enjoy every single day.

---

## 🛠️ My Tech Stack

* **Languages:** Python, JavaScript, Triton, C
* **Web Dev:** React, Node.js, MERN
* **Machine Learning:** PyTorch, Scikit-Learn 
* **Tools:** Git, GitHub, VS Code

---

## 🌱 Currently Learning

* Advanced Frontend & Backend web technologies
* Deep Learning & Artificial Intelligence concepts
* Modern developer tools and best practices

---

## 📈 GitHub Stats

![O-tech477's GitHub stats](https://github-readme-stats.vercel.app/api?username=O-tech477&show_icons=true&theme=radical&v=1)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=O-tech477&layout=compact&theme=radical&v=1)

---

## 🐍 My Contribution Snake

<div align="center">
  <img src="https://raw.githubusercontent.com/O-tech477/O-tech477/output/github-contribution-grid-snake.svg" alt="Snake animation" />
</div>

---

## 📫 Let's Connect!

<a href="https://www.linkedin.com/in/ananth-ram-omkar-k-prabhu-1b6a25319/" target="_blank">
  <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
</a>
<a href="https://www.instagram.com/ananth.ram.omkar/" target="_blank">
  <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" alt="Instagram"/>
</a>
